...
globalThis.playMode = centerTab === "play";
	...
	if (!liveHandlersRef.current) {
		...
		liveHandlersRef.current = {
			...
			load_motion: async (args) => {
				...
				await liveStateRef.current.loadMotion(args.url, prompt, undefined, args.drop ?? null, args.characterId);
	...
	async function loadMotion(url, prompt, rotationDeg = charA.rot, drop = null, targetCharacterId = activeChar.id) {
		setMotionBusy(true);
		setMotionError("");
		try {
