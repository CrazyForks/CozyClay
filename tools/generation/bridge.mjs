  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/generation/jobs") {
        ...
        const timestamp = now();
        const job = { id: randomUUID(), provider: spec.provider, model: spec.model, providerJobId: submitted.providerJobId, status: "processing", progress: null, outputPath: null, remoteOutputUrl: null, failure: null, warnings: submitted.warnings, estimatedCostUsd: provider.estimateCost(spec), createdAt: timestamp, updatedAt: timestamp };
        await store.set(job);
